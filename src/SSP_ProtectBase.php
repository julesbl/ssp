...
/**
 * @property SSP_Configuration $cfg - configuration object
 * @property SSP_DB $db - database object
 */
abstract class SSP_ProtectBase{
	...
	/** @var string short user name for login, boards etc. */
	var $userName = "";
	var $userAccessLevel = ""; // users access level for protection.
	var $userEmail = ""; // users email
	var $userId = ""; // unique id used to identify this user
	var $loggedIn = false; //user is logged in
	var $admin = false; // user is admin level
	var $sessionToken; // SID being used by the users session
	var $country; // country for user
	var $userInfo; // full user information object
	var $miscInfo; // full misc info object
	var $error = false; // logged in session fault
	var $errors = array(); // list of errors
	/** @var language code for translation */
	public $lang;

	var $noLoginDivertText = "fail";
	...
	/** @var SSP_translate translator object use to translate strings */
    ...
    public function __construct($pageAccessLevel="", $pageCheckEquals=false, $doHistory = true, $config = false){
        ...
        if($this->config->pageValid >0){
            header("Expires: ".gmstrftime("%a, %d %b %Y %H:%M:%S",gmmktime()+$pageValid)." GMT");
        }
	...
	private function setupLanguage(){
		if($this->cfg->translate){
			...
			if($this->lang != $this->cfg->lang){
				...
				SSP_Template::addPath(SSP_TEMPLATEPATHBASE. $this->lang. '/');
			}
		}
		
		// set up template path for default language
		SSP_Template::addPath(SSP_TEMPLATEPATHBASE. $this->cfg->lang. '/');
